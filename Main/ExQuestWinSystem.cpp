...
void ExWinQuestSystem::BindImages()
{
	...
	gInterface.BindObject(OPEN_QUEST, 0x7903, 25, 40, -1, -1);
}
// ----------------------------------------------------------------------------------------------
...
void ExWinQuestSystem::DrawOpenWQ()
{
	...
	gInterface.DrawGUI(OPEN_QUEST, 010, 200);

	if (gInterface.IsWorkZone(OPEN_QUEST))
	{

		gInterface.DrawToolTip(060, 210, "WINDOWS QUEST");
		// ----
		if (gInterface.Data[OPEN_QUEST].OnClick)
		{
			gInterface.DrawColoredGUI(OPEN_QUEST, 010, 200, pMakeColor(40, 20, 3, 130));
			return;
		}
		// ----
		gInterface.DrawColoredGUI(OPEN_QUEST, 010, 200, pMakeColor(255, 204, 20, 200));
	}
