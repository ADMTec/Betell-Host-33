...
char * Map101 = "NewMap101.tga";
char * Map102 = "NewMap102.tga";

char* LoadMapName(signed int MapNumber)
{
	if (MapNumber >= 82 && MapNumber <= 102)
	{
...
Naked(LoadInterfaceMapName)
{
	_asm
	{
			...
			// ----
			//----
			MOV DWORD PTR SS:[EBP-0x848],0x65		// 100 leothap Custom Map
			PUSH Map100								// Interface Name 
			LEA EAX,DWORD PTR SS:[EBP-0x28]
			PUSH EAX
			LEA ECX,DWORD PTR SS:[EBP-0x864]
			...
			// ----
			//----
			MOV DWORD PTR SS:[EBP-0x848],0x66		// 100 leothap Custom Map
			PUSH Map100								// Interface Name 
			LEA EAX,DWORD PTR SS:[EBP-0x28]
			PUSH EAX
			LEA ECX,DWORD PTR SS:[EBP-0x864]
