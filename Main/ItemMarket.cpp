...
void CItemMarket::Bind()
{
	...
	gInterface.BindObject(eMARKET_ITEM_BG, 0x9327, 152, 124, -1, -1);
...
void CItemMarket::DrawShop()
{
	...
	for(int i = this->m_ScrollPos; i < MARKET_ITEM_MAX; i++)
	{
		...
		{
			gInterface.DrawFormat(dwColor, ContentX + 30 + 5, ContentY + 5, 170, 3, "%s", GetItemName(this->m_Info[i].ItemType));
		}
	...
	for(int i = this->m_ScrollPos; i < MARKET_ITEM_MAX; i++)
	{
		...
		{
			gInterface.DrawFormat(dwColor, ContentX + 30 + 14, ContentY + 5, 170, 4, "Coin");
		}
		else if(this->m_Info[i].PriceType == eMarketPriceTypeCredit)
		{
			gInterface.DrawFormat(dwColor, ContentX + 30 + 14, ContentY + 5, 170, 4, "Ruud");
		}		
	...
	ContentY = gInterface.Data[eMARKET_FOOTER].Y - 68;
...
void CItemMarket::DrawPrice()
{
	...
	gInterface.DrawCheckLineEx(this->m_SelectItemPriceType1, eMARKET_PRICE_POINT, eMARKET_PRICE_CHECK1, eMARKET_PRICE_LINE, ContentX, ContentY, eWhite, "Price: Zen");

	ContentY += 20;

	gInterface.DrawCheckLineEx(this->m_SelectItemPriceType2, eMARKET_PRICE_POINT, eMARKET_PRICE_CHECK2, eMARKET_PRICE_LINE, ContentX, ContentY, eWhite, "Price: Coin");

	ContentY += 20;

	gInterface.DrawCheckLineEx(this->m_SelectItemPriceType3, eMARKET_PRICE_POINT, eMARKET_PRICE_CHECK3, eMARKET_PRICE_LINE, ContentX, ContentY, eWhite, "Price: Ruud");
...
void CItemMarket::DrawFilter(float PosX, float PosY)
{
	...
	gInterface.DrawCheckLineEx(this->m_FilterPriceType1, eMARKET_FILTER_POINT, eMARKET_FILTER_CHECK1, eMARKET_FILTER_LINE, ContentX, ContentY, eWhite, "Price: Zen");
			
	ContentY += 20;

	gInterface.DrawCheckLineEx(this->m_FilterPriceType2, eMARKET_FILTER_POINT, eMARKET_FILTER_CHECK2, eMARKET_FILTER_LINE, ContentX, ContentY, eWhite, "Price: Coin");
			
	ContentY += 20;

	gInterface.DrawCheckLineEx(this->m_FilterPriceType3, eMARKET_FILTER_POINT, eMARKET_FILTER_CHECK3, eMARKET_FILTER_LINE, ContentX, ContentY, eWhite, "Price: Ruud");
	...
	gInterface.DrawCheckLineEx(this->m_FilterLuck, eMARKET_FILTER_POINT, eMARKET_FILTER_CHECK4, eMARKET_FILTER_LINE, ContentX, ContentY, eWhite, "Luck Option +25%%");
			
	ContentY += 20;

	gInterface.DrawCheckLineEx(this->m_FilterExl, eMARKET_FILTER_POINT, eMARKET_FILTER_CHECK5, eMARKET_FILTER_LINE, ContentX, ContentY, eWhite, "Exellent Option");
			
	ContentY += 20;

	gInterface.DrawCheckLineEx(this->m_FilterAnc, eMARKET_FILTER_POINT, eMARKET_FILTER_CHECK6, eMARKET_FILTER_LINE, ContentX, ContentY, eWhite, "Ancent Option");
...
void CItemMarket::DrawTest(float PosX, float PosY, int ItemID)
{
	...
	gInterface.DrawItem2(flItemX, flItemY, barWidth, barHeight, this->m_Info[ItemID].ItemType, SET_ITEMOPT_LEVEL(this->m_ItemEx.Level), this->m_ItemEx.ExcellentOption, this->m_ItemEx.AncientOption, 0);
...
bool CItemMarket::Keyboard(DWORD Event)
{
	/*if(!gInterface.CheckWindowEx(ObjWindowsEx::exWinItemMarket))
	{
		if(Event == VK_NUMPAD9)
