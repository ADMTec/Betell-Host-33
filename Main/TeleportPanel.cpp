...
void ExTeleportEvo::LoadImages()
{
	...
	pLoadImage("Custom\\Interface\\TeleportPanel.tga", ex_EXTELEPORT_TITLE, 0x2601, 0x2900, 1, 0);
}
// ----------------------------------------------------------------------------------------------
...
void ExTeleportEvo::Draw()
{
	...
	if(this->ClickPage >= 0 && this->ClickPage < MAXEXTELEPORT)
	{
		...
		gInterface.DrawFormat(eAncient, ListX + 63, ListY + 5 + PlusY, 100, 3, gObjUser.GetMapName(this->data[i].map));
		...
		if(this->data[i].premium > 0)
		{
			...
			gInterface.DrawGUI(eTELEPORT_INFOBG, StartX + 30, ListY + PlusY);
			gInterface.DrawFormat(eYellow, StartX + 35, ListY + 5 + PlusY, 70, 3, "AccountLevel:");
		...
		if(this->data[i].bonus > 0)
		{
			...
			gInterface.DrawGUI(eTELEPORT_INFOBG, StartX + 30, ListY + PlusY);
			gInterface.DrawFormat(eYellow, StartX + 35, ListY + 5 + PlusY, 70, 3, "Coin:");
		...
		if(this->data[i].credit > 0)
		{
			...
			gInterface.DrawGUI(eTELEPORT_INFOBG, StartX + 30, ListY + PlusY);
			gInterface.DrawFormat(eYellow, StartX + 35, ListY + 5 + PlusY, 70, 3, "Ruud:");
	...
	else
	{
		...
		for(int i = 0; i < this->iLoadedCount; i++)
		{
			...
			gInterface.DrawFormat(eAncient, ListX  + 63, ListY + ply + 4 + i * 22, 100, 3, gObjUser.GetMapName(this->data[i].map));
...
void ExTeleportEvo::CGResult(BYTE num)
{
	...
	pRequest.h.set((LPBYTE)&pRequest, 0xFF, 0x28, sizeof(pRequest));
...
void ExTeleportEvo::CGDialog()
{
	...
	pMsg.h.set((LPBYTE)&pMsg, 0xFF, 0x29, sizeof(pMsg));
	pMsg.result = 1;
