...
void CLuckySpin::CGLuckySpinSend()
{
	...
	pRequest.h.set(0xFF,0x25,sizeof(pRequest));
...
void CLuckySpin::Draw()
{
	...
	pDrawImage(iGfxDialoge2, StartX - 30, StartY, 310, 340, 1, 1, 1.0, 1.0, 1, 1, 0); //223 280
	gFont[TAHOMA_BOLD_16]->DrawText(StartX + 103,StartY + 23, Fcolor->Gold, Fcolor->Trans, 15, (LPINT)5, gCustomMessage.GetMessage(80));
	//
	...
	if(this->RollNumber >= 0)
	{
		gFont[TAHOMA_BOLD_14]->DrawText(StartX + 100,StartY + 140, Fcolor->Orange, Fcolor->Trans, 15, (LPINT)5, GetItemName(ITEM(this->itemdata[RollNumber].ItemType,this->itemdata[RollNumber].ItemIndex)));
	...
	gFont[TAHOMA_BOLD_15]->DrawText(StartX + 120,StartY + 225, Fcolor->White, Fcolor->Trans, 15, (LPINT)5, gCustomMessage.GetMessage(81));

	gInterface.DrawGUI(eLuckySpinClose,StartX + 240, StartY + 15);
	if( gInterface.IsWorkZone3(eLuckySpinClose) )
	{
...
void CLuckySpin::EventWindow_Main(DWORD Event)
{
	...
	if (gInterface.IsWorkZone3(eLuckySpinRoll))
	{
		...
		RollNumber = -1;
		CG_LUCKYSPIN_REQ pRequest;
		pRequest.h.set(0xFF,0x31,sizeof(pRequest));
		DataSend((BYTE*)&pRequest,pRequest.h.size);
	}
