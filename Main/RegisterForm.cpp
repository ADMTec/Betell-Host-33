...
void CRegisterForm::Create(BYTE* thisa)
{
	...
	sprInputBoxCaptcha->SetFont(g_hFontBold);
...
void CRegisterForm::RenderForm()
{
	...
	g_pRenderText.RenderText(x - 32, y + 117, "Código per.", 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.RenderText(x - 32, y + 136, "Codigo Capcha", 90, 0, RT3_SORT_RIGHT);
	g_pRenderText.SetFont(g_hFontBold);
...
void CRegisterForm::RequestRegister(LPVOID pClass)
{
	...
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Por favor insira o capcha!");
	}
	else if(strcmp(szth->reCaptcha, szCaptcha) != NULL)
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "O código de Capcha não esta correto!");
	}
	else
	{
		if(CheckName(szID))
...
void CRegisterForm::RequestAccount(BYTE code)
{
	...
	switch(code)
	{
		case CREATE_ACCOUNT_FAIL_ID:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "O nome de usuário já existe");
			break;
		case CREATE_ACCOUNT_SUCCESS:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Conta registrada com sucesso");
			break;
		case CREATE_ACCOUNT_FAIL_RESIDENT:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "O sistema de criação de contas não esta disponivel neste momento");
			break;
		default:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Erro desconhecido");
			break;
	}
}
#endif